...
grep -v -e '\.[acho]$' -e '\.l[ao]$' -e '/aclocal/' \
	...
	-e '^/usr/lib/python' -e '^/usr/lib/ruby' -e '^/(mingw|clang(arm)?)../lib/python' \
	-e '^/usr/share/subversion' \
	-e '^/etc/skel/' -e '^/(mingw|clang(arm)?)../etc/skel/' \
	-e '^/usr/bin/svn' \
	-e '^/usr/bin/xml.*exe$' \
	-e '^/usr/bin/xslt.*$' \
	-e '^/usr/bin/b*zmore$' \
	...
	-e '^/(mingw|clang(arm)?)../share/doc/lib' \
	-e '^/(mingw|clang(arm)?)../share/doc/pcre2\?/' \
	-e '^/(mingw|clang(arm)?)../share/doc/git-doc/.*\.txt$' \
	-e '^/(mingw|clang(arm)?)../share/doc/zstd/' \
	-e '^/(mingw|clang(arm)?)../lib/gettext/' -e '^/(mingw|clang(arm)?)../share/gettext/' \
	-e '^/usr/include/' -e '^/(mingw|clang(arm)?)../include/' \
	-e '^/usr/share/doc/' \
	-e '^/usr/share/info/' -e '^/(mingw|clang(arm)?)../share/info/' \
	-e '^/mingw32/bin/lib\(ffi\|tasn1\)-.*\.dll$' \
	-e '^/(mingw|clang(arm)?)../share/git-doc/technical/' \
	-e '^/(mingw|clang(arm)?)../lib/cmake/' \
	-e '^/(mingw|clang(arm)?)../itcl/' \
	-e '^/(mingw|clang(arm)?)../t\(cl\|k\)[^/]*/\(demos\|msgs\|encoding\|tzdata\)/' \
	-e '^/(mingw|clang(arm)?)../bin/\(autopoint\|[a-z]*-config\)$' \
	-e '^/(mingw|clang(arm)?)../bin/lib\(asprintf\|brotlienc\|gettext\|gnutls\|gnutlsxx\|gmpxx\|pcre[013-9a-oq-z]\|pcre2-[13p]\|quadmath\|stdc++\|zip\)[^/]*\.dll$' \
	-e '^/(mingw|clang(arm)?)../bin/lib\(atomic\|charset\|gomp\|systre'"$EXCLUDE_LIBSSP"'\)-[0-9]*\.dll$' \
	-e '^/(mingw|clang(arm)?)../bin/\(asn1\|gnutls\|idn\|mini\|msg\|nettle\|ngettext\|ocsp\|pcre\|rtmp\|xgettext\|zip\)[^/]*\.exe$' \
	-e '^/(mingw|clang(arm)?)../bin/recode-sr-latin.exe$' \
	-e '^/(mingw|clang(arm)?)../bin/\(cert\|p11\|psk\|srp\)tool.exe$' \
	-e '^/(mingw|clang(arm)?)../.*/git-\(remote-testsvn\|shell\)\.exe$' \
	-e '^/(mingw|clang(arm)?)../.*/git-cvsserver.*$' \
	-e '^/(mingw|clang(arm)?)../.*/gitweb/' \
	-e '^/(mingw|clang(arm)?)../lib/\(dde\|itcl\|sqlite\|tdbc\)' \
	-e '^/(mingw|clang(arm)?)../libexec/git-core/git-archimport$' \
	-e '^/(mingw|clang(arm)?)../share/doc/git-doc/git-archimport' \
	-e '^/(mingw|clang(arm)?)../libexec/git-core/git-cvsimport$' \
	-e '^/(mingw|clang(arm)?)../share/doc/git-doc/git-cvsexport' \
	-e '^/(mingw|clang(arm)?)../libexec/git-core/git-cvsexport' \
	-e '^/(mingw|clang(arm)?)../share/doc/git-doc/git-cvsimport' \
	-e "^\\($(echo $EXCLUDE_MISSING_BUILTINS | sed 's/ /\\|/g')\\)\$" \
	-e '^/(mingw|clang(arm)?)../share/gtk-doc/' \
	-e '^/(mingw|clang(arm)?)../share/nghttp2/' \
...
then
	...
	grep -v \
		-e '^/(mingw|clang(arm)?)../share/locale/' \
		-e '^/(mingw|clang(arm)?)../share/git\(k\|-gui\)/lib/msgs/' \
		-e '^/usr/share/locale/'
fi |
if test -z "$MINIMAL_GIT"
then
	...
	grep -v -e '^/cmd/start-.*$' -e '^/cmd/\(git-gui\|gitk\).exe$' \
		...
		-e '^/git-\(bash\|cmd\)\.exe$' \
		-e '^/(mingw|clang(arm)?)../bin/\(certtool\.exe\|create-shortcut\.exe\)$' \
		-e '^/(mingw|clang(arm)?)../bin/\(curl\.exe\|envsubst\.exe\|gettext\.exe\)$' \
		-e '^/(mingw|clang(arm)?)../bin/.*-\(inflate\|deflate\)hd\.exe$' \
		-e '^/(mingw|clang(arm)?)../bin/\(gettext\.sh\|gettextize\)$' \
		-e '^/(mingw|clang(arm)?)../bin/\(gitk\|git-upload-archive\.exe\)$' \
		-e '^/(mingw|clang(arm)?)../bin/libgcc_s_seh-.*\.dll$' \
		-e '^/(mingw|clang(arm)?)../bin/libjemalloc\.dll$' \
		-e '^/(mingw|clang(arm)?)../bin/lib\(ffi\|gmp\|gomp\|jansson\|metalink\|minizip\|tasn1\)-.*\.dll$' \
		-e '^/(mingw|clang(arm)?)../bin/libvtv.*\.dll$' \
		-e '^/(mingw|clang(arm)?)../bin/libpcreposix.*\.dll$' \
		-e '^/(mingw|clang(arm)?)../bin/\(.*\.def\|update-ca-trust\)$' \
		-e '^/(mingw|clang(arm)?)../bin/\(openssl\|p11tool\|pkcs1-conv\)\.exe$' \
		-e '^/(mingw|clang(arm)?)../bin/\(psktool\|recode-.*\|sexp.*\|srp.*\)\.exe$' \
		-e '^/(mingw|clang(arm)?)../bin/\(WhoUses\|xmlwf\)\.exe$' \
		-e '^/(mingw|clang(arm)?)../etc/pki' -e '^/(mingw|clang(arm)?)../lib/p11-kit/' \
		-e '/git-\(add--interactive\|archimport\|citool\|cvs.*\)$' \
		-e '/git-\(difftool.*\|gui.*\|instaweb\|p4\|relink\)$' \
		-e '/git-\(send-email\|svn\)$' \
		-e '/(mingw|clang(arm)?)../libexec/git-core/git-\(imap-send\|daemon\)\.exe$' \
		-e '/(mingw|clang(arm)?)../libexec/git-core/git-remote-ftp.*\.exe$' \
		-e '/(mingw|clang(arm)?)../libexec/git-core/git-http-backend\.exe$' \
		-e "/(mingw|clang(arm)?)../libexec/git-core/git-\\($(sed \
			-e 's/^git-//' -e 's/\.exe$//' -e 's/$/\\/' \
				</$MSYSTEM_LOWER/share/git/builtins.txt |
			tr '\n' '|')\\)\\.exe\$" \
		-e '^/(mingw|clang(arm)?)../share/doc/nghttp2/' \
		-e '^/(mingw|clang(arm)?)../share/gettext-' \
		-e '^/(mingw|clang(arm)?)../share/git/\(builtins\|compat\|completion\)' \
		-e '^/(mingw|clang(arm)?)../share/git/.*\.ico$' \
		-e '^/(mingw|clang(arm)?)../share/\(git-gui\|gitweb\)/' \
		-e '^/(mingw|clang(arm)?)../share/perl' \
		-e '^/(mingw|clang(arm)?)../share/pki/' \
		-e '/zsh/' \
